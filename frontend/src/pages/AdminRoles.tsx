...
const AdminRoles = () => {
  ...
  const openCreate = () => { setSelectedRole(null); setForm(emptyForm()); setModalOpen(true); };
  const openEdit = async (role: Role) => {
    setSelectedRole(role); setModalOpen(true);
    try {
      const loaded = (await adminApi.getRole(role.id)).data as Role;
      setForm({ name: loaded.name, description: loaded.description ?? '', permissionNames: loaded.permissionNames });
    } catch (error) { setModalOpen(false); addToast('error', messageFrom(error, 'Failed to load role.')); }
  };
  const togglePermission = (permission: string) => setForm((current) => ({ ...current, permissionNames: current.permissionNames.includes(permission) ? current.permissionNames.filter((value) => value !== permission) : [...current.permissionNames, permission] }));
  const save = async () => {
    if (!form.name.trim()) { addToast('error', 'A role name is required.'); return; }
    setSaving(true);
    try {
      ...
      setModalOpen(false); await loadRoles();
    } catch (error) { addToast('error', messageFrom(error, 'Failed to save role.')); }
  ...
  return <div>
    ...
    <Modal isOpen={modalOpen} onClose={() => !saving && setModalOpen(false)} title={selectedRole ? t('roles.editRole') : t('roles.createRole')}><div className="space-y-4"><input aria-label={t('common.name')} value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} className="w-full px-3 py-2 border rounded-md" placeholder={t('common.name')} /><textarea aria-label={t('common.description')} value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} className="w-full px-3 py-2 border rounded-md" placeholder={t('common.description')} /><div className="max-h-80 overflow-y-auto space-y-4">{Object.entries(PERMISSION_GROUPS).map(([group, permissions]) => <fieldset key={group}><legend className="font-medium">{group}</legend><div className="grid grid-cols-1 sm:grid-cols-2 gap-2">{permissions.map((permission) => <label key={permission} className="flex gap-2 text-sm"><input type="checkbox" checked={form.permissionNames.includes(permission)} onChange={() => togglePermission(permission)} />{permission}</label>)}</div></fieldset>)}</div><div className="flex justify-end gap-2"><button disabled={saving} onClick={() => setModalOpen(false)} className="px-4 py-2 border rounded-md">{t('common.cancel')}</button><button disabled={saving} onClick={() => void save()} className="px-4 py-2 bg-blue-600 text-white rounded-md">{t('common.save')}</button></div></div></Modal>
  </div>;
};
