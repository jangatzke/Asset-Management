...
const workspaces: Array<{ key: Workspace; resources: string[] }> = [
  { key: 'training', resources: ['trainingCourses', 'trainingAssignments', 'trainingAcknowledgements'] },
  { key: 'metrics', resources: ['metricDefinitions', 'metricValues'] },
  { key: 'reviews', resources: ['managementReviews', 'managementReviewActions'] },
  { key: 'workflows', resources: ['workflowDefinitions', 'workflowInstances', 'workflowTasks'] },
  { key: 'reports', resources: ['reportDefinitions', 'reportRuns', 'exportJobs'] },
];
...
function Training({ courses, assignments, acknowledgements, open }: any) { const { t } = useI18n(); return <div className="grid gap-5 lg:grid-cols-2"><List heading={t('operationsWorkspace.sections.courses')} items={courses}><Button onClick={() => open('course')}>{t('operationsWorkspace.actions.createCourse')}</Button></List><List heading={t('operationsWorkspace.sections.assignments')} items={assignments}><div className="flex gap-2"><Button onClick={() => open('assignment')}>{t('operationsWorkspace.actions.assignCourse')}</Button><Button onClick={() => open('completion')}>{t('operationsWorkspace.actions.recordCompletion')}</Button></div></List><List heading={t('operationsWorkspace.sections.acknowledgements')} items={acknowledgements}><Button onClick={() => open('acknowledgement')}>{t('operationsWorkspace.actions.acknowledgeCourse')}</Button></List></div> }
function Metrics({ definitions, values, open }: any) { const { t } = useI18n(); return <div className="grid gap-5 lg:grid-cols-2"><List heading={t('operationsWorkspace.sections.metricDefinitions')} items={definitions}><Button onClick={() => open('metric')}>{t('operationsWorkspace.actions.defineMetric')}</Button></List><List heading={t('operationsWorkspace.sections.valuesTrendsBreaches')} items={values}><Button onClick={() => open('value')}>{t('operationsWorkspace.actions.enterValue')}</Button></List></div> }
function Reviews({ reviews, actions, open }: any) { const { t } = useI18n(); return <div className="grid gap-5 lg:grid-cols-2"><List heading={t('operationsWorkspace.workspaces.reviews')} items={reviews}><div className="flex gap-2"><Button onClick={() => open('review')}>{t('operationsWorkspace.actions.scheduleReview')}</Button><Button onClick={() => open('approve')}>{t('operationsWorkspace.actions.approveReview')}</Button></div></List><List heading={t('operationsWorkspace.sections.decisionsAndActions')} items={actions}><Button onClick={() => open('reviewAction')}>{t('operationsWorkspace.actions.addAction')}</Button></List></div> }
function Workflows({ definitions, instances, tasks, open }: any) { const { t } = useI18n(); return <div className="grid gap-5 lg:grid-cols-2"><List heading={t('operationsWorkspace.sections.administrativeDefinitions')} items={definitions}/><List heading={t('operationsWorkspace.sections.contextualInstances')} items={instances}><Button onClick={() => open('startWorkflow')}>{t('operationsWorkspace.actions.startFromEntity')}</Button></List><List heading={t('operationsWorkspace.sections.actionableTasks')} items={tasks}><Button onClick={() => open('transition')}>{t('operationsWorkspace.actions.transitionInstance')}</Button></List></div> }
function Reports({ definitions, runs, exports, open }: any) { const { t } = useI18n(); return <div className="grid gap-5 lg:grid-cols-2"><List heading={t('operationsWorkspace.sections.reportDefinitions')} items={definitions}><Button onClick={() => open('report')}>{t('operationsWorkspace.actions.createDefinition')}</Button></List><List heading={t('operationsWorkspace.sections.runResults')} items={runs}><Button onClick={() => open('run')}>{t('operationsWorkspace.actions.runReport')}</Button></List><List heading={t('operationsWorkspace.sections.exports')} items={exports}/></div> }
...
function OperationModal({ kind, courses, assignments, definitions, reviews, workflows, instances, reports, onClose, onDone }: any) {
  ...
  const choose = (items: RecordItem[], label: string) => <label className="block text-sm font-medium">{label}<select required className={inputClass} onChange={e => setSelected(items.find(item => item.id === e.target.value) ?? null)} defaultValue=""><option value="" disabled>{t('operationsWorkspace.form.selectOption')}</option>{items.map(item => <option key={item.id} value={item.id}>{item.title ?? item.name ?? item.displayId}</option>)}</select></label>;
  const needsUser = ['course', 'assignment', 'metric', 'review', 'reviewAction', 'report'].includes(kind); const picker = needsUser && <EntityPicker label={t('operationsWorkspace.form.responsibleUser')} labelKey="entityPicker.searchPlaceholder" entityType="user" value={user} onChange={setUser} required />;
