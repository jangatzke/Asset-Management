...
function OperationModal({ kind, courses, assignments, definitions, reviews, workflows, instances, reports, onClose, onDone }: any) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = new FormData(event.currentTarget); const str = (key: string) => String(form.get(key) ?? ''); const num = (key: string) => Number(form.get(key));
    ...
    if (kind === 'completion' && selected) await phase6Api.completeTraining(selected.id, { score: num('score') || undefined, result: 'passed' });
    if (kind === 'acknowledgement' && selected) await phase6Api.acknowledgeTraining({ courseId: selected.id, comment: str('comment') || undefined });
    if (kind === 'metric' && user) await phase6Api.createMetricDefinition({ name: str('name'), unit: str('unit') || undefined, ownerId: user.id, thresholds: { warningMin: num('warningMin') || undefined, warningMax: num('warningMax') || undefined, criticalMin: num('criticalMin') || undefined, criticalMax: num('criticalMax') || undefined } });
    ...
    if (kind === 'startWorkflow' && selected && entity) await phase6Api.startWorkflow({ definitionId: selected.id, entityType: str('entityType'), entityId: entity.id });
