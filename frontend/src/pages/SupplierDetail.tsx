...
const button = 'rounded bg-primary-600 px-3 py-2 text-sm font-medium text-white hover:bg-primary-700 disabled:opacity-50';
...
export default function SupplierDetailPage() {
  ...
  return <div className="space-y-6">
    <header className="flex flex-wrap items-start justify-between gap-3"><div><button onClick={() => navigate('/isms-operations')} className="text-sm text-primary-600">← {t('ismsOperations.title')}</button><h1 className="mt-2 text-2xl font-bold">{String(detail.supplier.legalName)}</h1><p className="text-sm text-gray-500">{String(detail.supplier.displayId)} · {t('suppliers.title')}</p></div><span className="rounded bg-blue-50 px-3 py-1 text-sm text-primary-800">{String(detail.supplier.status)}</span></header>
    {error && <p className="rounded bg-red-50 p-3 text-red-800">{error}</p>}
    <nav className="flex flex-wrap gap-2">{(['profile', 'assessments', 'contracts', 'risks', 'actions', 'history'] as Tab[]).map((item) => <button key={item} onClick={() => setTab(item)} className={`rounded px-3 py-2 text-sm capitalize ${tab === item ? 'bg-primary-600 text-white' : 'bg-gray-100 dark:bg-gray-700'}`}>{item === 'actions' ? 'CAPAs / Actions' : item === 'history' ? 'History / Reviews' : item}</button>)}</nav>
...
function Editor<T>({ title, items, onAdd, render }: { title: string; items: T[]; onAdd: () => void; render: (item: T, index: number) => React.ReactNode }) { return <section><div className="flex justify-between"><h3 className="font-medium">{title}</h3><button type="button" className="text-sm text-primary-600" onClick={onAdd}>Add</button></div>{items.map(render)}</section>; }
