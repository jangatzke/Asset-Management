...
export default function AdminEmailGateway() {
  ...
  return <div className="mx-auto max-w-6xl space-y-6 p-4 sm:p-6">
    ...
    <div className="flex flex-wrap gap-3"><button onClick={save} disabled={saving} className="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50">{saving ? t('emailGateway.saving') : t('emailGateway.saveConfig')}</button><button onClick={() => void run(t('emailGateway.testInbound'), emailGatewayAdminApi.testInbound)} className="rounded-md border px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-800">{t('emailGateway.testInbound')}</button><button onClick={() => void run(t('emailGateway.testSmtp'), emailGatewayAdminApi.testSmtp)} className="rounded-md border px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-800">{t('emailGateway.testSmtp')}</button><button onClick={() => void run(t('emailGateway.pollNow'), emailGatewayAdminApi.pollNow)} className="rounded-md border px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-800">{t('emailGateway.pollNow')}</button></div>
