...
const IncidentDetail = () => {
  ...
  return <div data-testid="incident-detail-page" className="space-y-6">
    ...
    <Section title="NIS2 significance and reportability"><p className="mb-2 text-sm">Significance: <strong>{incident.isSignificant ? 'Significant' : 'Not significant'}</strong></p><p className="mb-4 text-sm text-gray-600">{incident.significanceReasons?.join(', ') || 'No rule reasons recorded.'}</p><div className="mb-4 text-sm">Latest assessment: {(incident.assessments[0] as any)?.isReportable === undefined ? 'Not assessed' : (incident.assessments[0] as any).isReportable ? 'Reportable' : 'Not reportable'}{!(incident.assessments[0] as any)?.isReportable && (incident.assessments[0] as any)?.decisionApprovedBy ? ` · approved by ${(incident.assessments[0] as any).decisionApprovedBy}` : ''}</div><div className="grid gap-3 md:grid-cols-2"><label className="flex gap-2"><input type="checkbox" checked={assessment.isReportable} onChange={(event) => setAssessment({ ...assessment, isReportable: event.target.checked })} /> Reportable</label><textarea value={assessment.isReportable ? assessment.reportingJustification : assessment.decisionNotToReport} onChange={(event) => setAssessment(assessment.isReportable ? { ...assessment, reportingJustification: event.target.value } : { ...assessment, decisionNotToReport: event.target.value })} placeholder={assessment.isReportable ? 'Reporting justification' : 'Decision not to report (required)'} className="rounded border p-2" />{!assessment.isReportable && <EntityPicker label="Approving user" labelKey="entityPicker.searchPlaceholder" entityType="user" value={decisionApprover} onChange={(approver) => { setDecisionApprover(approver); setAssessment({ ...assessment, decisionApprovedBy: approver.id }); }} required />}</div><button data-testid="incident-save-assessment" disabled={busy || !user?.id || (!assessment.isReportable && !assessment.decisionApprovedBy)} onClick={() => run(() => incidentApi.assess(incident.id, { ...assessment, assessorId: user!.id }), 'Reportability assessment saved.')} className="mt-3 rounded bg-blue-600 px-4 py-2 text-white disabled:opacity-50">Save assessment</button></Section>
