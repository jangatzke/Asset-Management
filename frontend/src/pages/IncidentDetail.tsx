...
const IncidentDetail = () => {
  ...
  return <div data-testid="incident-detail-page" className="space-y-6">
    ...
    <Section title="Status transition"><p className="mb-3 text-sm text-gray-600">Current status: <strong>{(incident.status || 'new').replace(/_/g, ' ')}</strong> · Status changes require a reason and are recorded in the incident history. Closing an incident requires the dedicated closure flow below.</p>{['resolved', 'closed'].includes(incident.status || 'new') ? <p className="text-sm text-gray-500">This incident is in a terminal workflow state and cannot be transitioned further.</p> : <div><div className="grid gap-2 md:grid-cols-3"><select value={statusChange.status} onChange={(event) => setStatusChange({ ...statusChange, status: event.target.value as '' | 'new' | 'under_investigation' | 'contained' | 'resolved' })} className="rounded border p-2"><option value="">Target status…</option>{['new', 'under_investigation', 'contained', 'resolved'].filter((status) => status !== incident.status).map((status) => <option key={status} value={status}>{status.replace(/_/g, ' ')}</option>)}</select><input value={statusChange.reason} onChange={(event) => setStatusChange({ ...statusChange, reason: event.target.value })} placeholder="Reason (required)" className="rounded border p-2 md:col-span-2" /></div><button disabled={busy || !statusChange.status || !statusChange.reason.trim()} onClick={() => { const targetStatus = statusChange.status; if (targetStatus) { run(() => incidentApi.changeStatus(incident.id, { status: targetStatus, reason: statusChange.reason }), 'Incident status changed.'); } }} className="mt-2 rounded bg-blue-600 px-4 py-2 text-white disabled:opacity-50">Change status</button></div>}</Section>
