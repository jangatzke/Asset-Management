...
export default function Tickets() {
  ...
  return <main id="main-content" className="mx-auto max-w-screen-2xl p-4 sm:p-6 lg:p-8">
    ...
    <section className="overflow-hidden rounded-lg bg-white shadow-sm dark:bg-gray-800"><div className="overflow-x-auto"><table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700"><thead className="bg-gray-50 dark:bg-gray-700"><tr>{['ticket', 'type', 'priority', 'status', 'slaTarget', 'reportedBy', 'created', 'updated'].map((heading) => <th key={heading} className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-600 dark:text-gray-200">{t(`tickets.headings.${heading}`)}</th>)}</tr></thead><tbody className="divide-y divide-gray-200 dark:divide-gray-700">{loading ? <tr><td colSpan={8} className="p-8 text-center text-gray-500">{t('tickets.loading')}</td></tr> : tickets.length === 0 ? <tr><td colSpan={8} className="p-8 text-center text-gray-500">{t('tickets.empty')}</td></tr> : tickets.map((ticket) => <tr key={ticket.id} className="hover:bg-gray-50 dark:hover:bg-gray-700"><td className="px-4 py-3"><Link to={`/tickets/${ticket.id}`} className="font-semibold text-blue-700 hover:underline dark:text-blue-300">{ticket.displayId}</Link><div className="max-w-sm truncate text-sm text-gray-700 dark:text-gray-200">{ticket.title}</div></td><td className="px-4 py-3 text-sm">{t(`tickets.types.${ticket.type}`)}</td><td className="px-4 py-3"><span className={badge(ticket.priority)}>{t(`tickets.priorities.${ticket.priority}`)}</span></td><td className="px-4 py-3"><span className={badge(ticket.status)}>{ticket.status.replace(/_/g, ' ')}</span></td><td className="px-4 py-3 text-sm">{ticket.resolutionDueAt ? new Date(ticket.resolutionDueAt).toLocaleString() : '—'}</td><td className="px-4 py-3 text-sm">{ticket.requester ? `${ticket.requester.firstName ?? ''} ${ticket.requester.lastName ?? ''}`.trim() || ticket.requester.email : '—'}</td><td className="px-4 py-3 text-sm">{ticket.openedAt ? new Date(ticket.openedAt).toLocaleString() : ticket.createdAt ? new Date(ticket.createdAt).toLocaleString() : '—'}</td><td className="px-4 py-3 text-sm">{new Date(ticket.updatedAt).toLocaleString()}</td></tr>)}</tbody></table></div></section>
