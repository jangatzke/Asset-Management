...
export const DataTableShell: React.FC<DataTableShellProps> = ({
  ...
  const [selectedDensity, setSelectedDensity] = useState<TableDensity | null>(null);
  const effectiveDensity = density ?? selectedDensity ?? (rowCount > 10 ? 'compact' : 'comfortable');
  ...
  return (
    <section data-table-density={effectiveDensity} className={`data-table-shell overflow-hidden rounded-lg bg-white shadow-sm dark:bg-gray-800 ${className}`}>
      {(toolbar || filters || onExport || (columns && visibleColumns && onVisibleColumnsChange) || rowCount > 0 || (density && onDensityChange)) && (
        <div className="border-b border-gray-200 px-4 py-2.5 dark:border-gray-700 sm:px-5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="ml-auto flex flex-wrap items-center gap-2">
              ...
              {columns && visibleColumns && onVisibleColumnsChange && <div className="relative"><button type="button" onClick={() => setColumnsOpen((open) => !open)} aria-expanded={columnsOpen} className="inline-flex items-center gap-1.5 rounded-md border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary-500 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700"><ViewColumnsIcon className="h-4 w-4" aria-hidden="true" />{columnsLabel}</button>{columnsOpen && <div className="absolute right-0 z-20 mt-2 w-56 rounded-md border border-gray-200 bg-white p-2 shadow-lg dark:border-gray-600 dark:bg-gray-800">{columns.filter((column) => column.hideable !== false).map((column) => <label key={column.key} className="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-sm text-gray-700 hover:bg-gray-50 dark:text-gray-200 dark:hover:bg-gray-700"><input type="checkbox" checked={visibleColumns.includes(column.key)} onChange={() => toggleColumn(column.key)} />{column.label}</label>)}</div>}</div>}
              <div className="relative"><button type="button" onClick={() => setDensityOpen((open) => !open)} aria-expanded={densityOpen} className="rounded-md border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary-500 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700">{densityLabel}: {effectiveDensity === 'compact' ? compactLabel : comfortableLabel}</button>{densityOpen && <div className="absolute right-0 z-20 mt-2 w-40 rounded-md border border-gray-200 bg-white p-1 shadow-lg dark:border-gray-600 dark:bg-gray-800">{(['comfortable', 'compact'] as const).map((option) => <button type="button" key={option} onClick={() => { if (onDensityChange) onDensityChange(option); else setSelectedDensity(option); setDensityOpen(false); }} className="block w-full rounded px-2 py-1.5 text-left text-sm text-gray-700 hover:bg-gray-50 dark:text-gray-200 dark:hover:bg-gray-700">{option === 'compact' ? compactLabel : comfortableLabel}</button>)}</div>}</div>
