...
## Phase 4 OIDC Authorization Architecture

The OIDC login path is consolidated around backend service state and `openid-client`. `GET /api/v1/auth/oidc/authorize` creates backend-bound `state`, `nonce`, and PKCE verifier values, stores only `stateHash` plus the verifier and nonce in `OidcLoginState`, and returns an authorization URL generated by `openid-client.buildAuthorizationUrl`. `POST /api/v1/auth/oidc/callback` consumes that state exactly once, calls `openid-client.authorizationCodeGrant` with expected `state`, `nonce`, and `pkceCodeVerifier`, then applies tenant and account-linking policy before issuing the standard refresh-token-backed session.

New persistence tables are `oidc_login_states` for short-lived authorization transaction state and `oidc_account_links` for provider/config subject links. `oidc_configs.clientSecretRef` points to secret material held outside the database, normally via environment variables; `oidc_configs.clientSecret` is legacy/deprecated compatibility data.
