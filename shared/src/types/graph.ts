// Graph types for asset dependency visualization and impact analysis (AST-011, AST-012)
...
export interface GraphNode {
  ...
  type: string; // Backward-compatible display/type field used by the frontend
  displayId?: string;
  assetType?: string;
  assetTypeId?: string;
  criticality?: string;
...
export interface GraphEdge {
  ...
  sourceId: string;
  targetId: string;
  source: string; // Backward-compatible alias for canvas graph components
  target: string; // Backward-compatible alias for canvas graph components
