...
- The backend imports runtime Zod schemas from `shared/dist`; `backend`'s `predev` builds the `shared` workspace first. Keep the root `package-lock.json` committed so a fresh `npm ci` installs backend dependencies such as `imapflow` reproducibly.
